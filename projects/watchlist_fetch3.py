...
# Global cookie-jar opener so crumb cookies persist across requests
...
def get_crumb():
    ...
    for host in ("query1","query2"):
        try:
            with OPENER.open(urllib.request.Request(f"https://{host}.finance.yahoo.com/v1/test/getcrumb", headers={'User-Agent':'Mozilla/5.0'}), timeout=15) as r:
...
def fetch_one(t):
    ...
    for host in ("query1","query2"):
        try:
            ...
            with OPENER.open(urllib.request.Request(url, headers={'User-Agent':'Mozilla/5.0'}), timeout=15) as r:
                data = json.load(r)
            break
        except Exception as e:
            out["chart_err"] = str(e)
...
print("TOTAL", len(results), "CRUMB_OK", bool(CRUMB), "CRUMB", CRUMB)
