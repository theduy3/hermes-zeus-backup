...
from life_store import write as ledger_write, OPERATIONAL_SCHEMA
_ROOT_ENV=os.environ.get('LIFE_OS_ROOT')
ROOT=Path(_ROOT_ENV)/'tracker' if _ROOT_ENV else Path(__file__).parent
DB=ROOT/'data/tracker.sqlite3'; LOCK=threading.RLock(); SEC=Path.home()/'.config/life-tracker/secrets.env'
def secrets_file():
...
def init():
 c=con();c.executescript(OPERATIONAL_SCHEMA)
...
def api(e,st):
 ...
 try:
  ...
  if p=='/api/complete' and m=='POST':
   ...
   ledger_write('tracker','habit_completion',d,{'habit_id':b['id'],'state':state},event_id='evt-'+secrets.token_hex(12),owner='tracker')
  ...
  ledger_write(domain,tab,selected,ledger_payload,event_id='evt-'+secrets.token_hex(12),owner='tracker')
