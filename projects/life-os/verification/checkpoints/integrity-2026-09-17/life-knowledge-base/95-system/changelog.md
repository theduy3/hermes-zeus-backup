...
## 2026-08-21 — Thor reMarkable planner captures (retro-documented 2026-09-17)
- Five health events were written to `30-health/logs/2026-08.md` at 2026-08-21T04:33:31Z with no changelog entry and no checkpoint at the time:
  - `thor-rm-2026-08-14`, `thor-rm-2026-08-17`, `thor-rm-2026-08-18`, `thor-rm-2026-08-19` — exercise done (reMarkable planner grid, X on p963, source=remarkable-2026-planner).
  - `med-rm-2026-08-20` — `meditation_done`, estimated, `source_ids=["remarkable_planner"]`; not written via `thor_log.py`.
- Content treated as legitimate self-report. Regularised by changelog plus checkpoint `integrity-2026-09-17`.

## 2026-09-17 — Integrity: lock, reconcile, lanes, merge corrections
- `write()` holds `fcntl.flock` on the ledger file across read-check-append-rebuild (in-process `WRITE_LOCK` kept). Appends `flush` + `fsync`.
- `ledger_digest()` hashes every file under the knowledge base. `reconcile()` compares cache payloads to ledger payloads and that digest; tampered cache or summary is not `ok`.
- `rebuild()` refuses to drop cache rows whose ids are missing from the ledger unless `drop_cache_only=True`.
- `rebuild_tracker()` upserts completions on `(habit_id, day)` in event order. Tests use the production schema including `unique(habit_id,day)`.
- `build.py` `w()` refuses to overwrite an existing file.
- `write()` requires `owner`. A `supersedes` id must exist and must not belong to a different owner. Capture CLIs pass `thor` / `finance` / `charles` / `life`; the tracker API uses `tracker`.
- A superseding event merges its predecessor payload by default. `None` clears a field. `replace=True` / `--replace` wholesale-replaces.
- `LIFE_OS_ROOT` selects an isolated tree. `tests/test_live.py` uses a throwaway root and a private server; it does not write the live ledger.
- `checkpoint()` also copies `tracker/*.py`. Checkpoint `integrity-2026-09-17` taken after this entry.
- Phase 5 Charles still waiting on real investment data; not started here.
