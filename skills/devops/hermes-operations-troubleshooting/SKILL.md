...
6. Changing the override alone is **not** enough. `npm install` may leave nested copies at the old version and mark them `invalid: "3.3.18"`. Force the nested installs + lockfile to settle:
   ...
   hermes doctor
   ```
7. Do **not** add overridden packages (`nanoid`, `browserslist`, `sanitize-html`, …) as root direct deps to force a bump. That yields `EOVERRIDE` and can leave lockfile/audit stale even when node_modules looks patched. Fix overrides only, then `npm install --no-fund --no-audit`.
