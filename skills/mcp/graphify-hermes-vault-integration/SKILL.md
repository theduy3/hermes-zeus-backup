...
- `default`: all Graphify MCP servers (`graphify-hermes`, `graphify-vault`, `graphify-vault-core`, `graphify-vault-sources`, `graphify-vault-daily`).
- `zeus`: all Graphify MCP servers.
- `finance`: `graphify-vault-core` + `graphify-vault-sources`.
- Business profiles (`butter`, `catthew`, `charles`): `graphify-vault-core` only.
- `thor`: no Graphify by default unless health/wellness vault lookup is requested.
- `wiki`: leave unchanged unless it needs graph-specific duties.
