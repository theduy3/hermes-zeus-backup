---
name: vault-research
description: "Use from Telegram as /vault_research <question> to run the verified vault research workflow: split the question, collect dated receipts across practitioner/social and official sources, run a skeptic gate, and write surviving findings into the vault with expiry dates."
version: 1.0.0
author: Zeus
license: MIT
metadata:
...
# Vault Research Telegram Command
...
Primary Telegram command:

```text
/research <question>
```

Fallback skill command:

```text
/vault_research <question>
```

This skill is intentionally named `vault-research` so Hermes auto-registers it as a slash-skill command. Telegram command names cannot contain hyphens, so Hermes maps `/vault_research` to the skill command `/vault-research`.

For the shorter `/research` command, create a Hermes `quick_commands` alias in the profile config:

```yaml
quick_commands:
  research:
    type: alias
    target: /vault_research
    description: Run verified vault research into the vault
```

Before telling the user the Telegram command is visible, verify both layers:

1. Hermes dispatch: `/research` aliases to `/vault_research`, and `/vault_research` resolves to `/vault-research`.
2. Telegram menu: Telegram Bot API `getMyCommands` includes `research`.

Pitfall: a Hermes skill command can work when typed manually but still be absent from Telegram's visible `/` picker because Telegram command menus are capped and Hermes trims lower-priority skill commands. If the menu is missing the command, ensure quick commands are included in Telegram BotCommand generation, then refresh BotCommand scopes (`default`, `all_private_chats`, `all_group_chats`) or restart the profile gateway.

If a gateway restart is unsafe or denied, call Telegram Bot API `setMyCommands` directly for those scopes to refresh visibility, then still note that code/config changes need a future gateway restart to persist inside the gateway process.
