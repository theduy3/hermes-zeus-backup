...
Individual Done-button tasks:
- Build a separate list of concrete tasks that are overdue or due today/tonight. Do NOT include routine daily/baseline items.
- Before final response, write this JSON file exactly: `/home/hermes/.hermes/profiles/zeus/task_buttons/today_tasks.json` with schema:
  `{"date": "YYYY-MM-DD", "tasks": [{"title": "clean task text", "source": "Obsidian|Catthew|Financial Advisor|Investment Strategist|Thor", "file_path": "/vault/Tasks/tasks/name.md if known, else empty"}]}`
- The follow-up button job will send each item individually with a ✅ Done button and log completion.
- Remove overdue/today/tonight tasks from the Daily Evening Task Prep final briefing. The final briefing should focus only on tomorrow and next-7-days preparation, excluding anything that will be sent as an individual Done-button task.
...
- Do not include overdue/today/tonight items in the final briefing; those belong only in the sidecar JSON for individual Done-button messages.
- Keep under one screen if possible; max 8 Tomorrow, max 5 Next 7 Days.
- Clean plaintext only; no raw Obsidian brackets/wiki-links.


## Response

# ✅ Tomorrow Prep — Tuesday, 2026-05-26

📌 Tomorrow
☐ Pay RBC Avion 7760 due date on the 3rd [1]

🏠 Family / Catthew
☐ Buy Korean pear and ghee [2]
☐ Prepare for Victoria music class on Wednesday, May 27 at 10:30 AM [3]

💰 Financial Advisor
☐ Close BMO Porter Mastercard by Monday, June 1 [4]

💪 Thor / Wellness
☐ Complete weekly measurements on Sunday, May 31 [5]

📅 Next 7 Days
☐ Pay TD Aeroplan Biz 9716 due on the 30th [6]

🕒 Timezone
☐ Keep tomorrow scheduling in Pacific Time; no active Quebec or East Coast trip detected [7]
