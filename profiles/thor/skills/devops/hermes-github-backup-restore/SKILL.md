...
- **Cron job**: `nightly-hermes-github-backup` fires at 05:00 UTC nightly
- **DST handling**: Self-checks America/Chicago midnight. During CST (winter, UTC-6) it waits 1 hour until midnight arrives at 06:00 UTC. During CDT (summer, UTC-5) it fires immediately. This means the sync always happens at true midnight Central.
- **What's backed up**: `~/.hermes/` minus secrets — skills, memory, config templates, plugins, and any custom files
- **What's excluded** (.gitignore): `.env`, `auth.json`, `*.pem`, `*.key`, `logs/`, `sessions/`, `audio_cache/`, `hermes-agent/` (upstream source, can re-clone), `__pycache__/`, `venv/`, `.venv/`, `node_modules/`, `cache/`, `tmp/`
