#!/usr/bin/env python3
"""Log a sitz bath button click into Thor's monthly sitz bath log."""
import sys
...
def main() -> int:
    ...
    home = hermes_home()
    log_path = home / "logs" / f"sitz-bath-log-{now:%Y-%m}.md"
    log_path.parent.mkdir(parents=True, exist_ok=True)
    text = log_path.read_text(encoding="utf-8") if log_path.exists() else f"# Sitz Bath Log — {now:%B %Y}\n"
    ...
    print(f"✅ Sitz bath done\n{minutes} min warm water logged.")
    return 0
