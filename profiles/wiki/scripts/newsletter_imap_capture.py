...
Intended for Hermes cron no_agent=True. Prints nothing when there are no new
messages, so cron stays silent. Marks messages seen only after the Inbox
...
ENV_PATH = Path("/home/hermes/.hermes/profiles/wiki/.env")
...
def main() -> int:
    ...
    with imaplib.IMAP4_SSL(host, port, timeout=30) as imap:
        ...
        for msg_id in ids:
            ...
            date_dt = received_dt(msg)
            received_iso = date_dt.isoformat().replace("+00:00", "Z")
            body_kind, body = extract_body(msg)
            body = body.strip() or "(No readable text body extracted.)"
            stamp = datetime.now().strftime("%Y-%m-%d-%H%M")
            slug = slugify(subject or from_)
            path = unique_path(INBOX_DIR / f"{stamp}-newsletter-{slug}.md")
            md = "\n".join([
                "---",
                "type: newsletter",
                "source: email",
            ...
            # Mark seen only after write succeeded.
            imap.store(msg_id, "+FLAGS", "\\Seen")
