...
- **Social/video URL extraction:** when a user sends a Facebook/Reels/TikTok/short-form video URL and asks to extract information, capture the URL to Inbox first, then download/preserve video + thumbnail + metadata under `Attachments/`, transcribe audio, enrich the Inbox source with extracted bullets/transcript, and ingest normally. See `references/social-video-extraction.md` for the proven `yt-dlp` + ffmpeg + Faster Whisper pattern and verification checklist.
