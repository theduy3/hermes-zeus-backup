...
- **Newsletter inbox ingestion:** the user's dedicated newsletter inbox is `theduy.newsletter@gmail.com`. For newsletter subscriptions, prefer a dedicated IMAP polling capture into `/vault/Inbox/` over the Hermes Email Gateway conversation adapter: the gateway is optimized for human email threads and may ignore bulk/list mail with `List-Unsubscribe`, `Precedence: bulk`, or noreply-style headers. Configure Gmail with a 16-character app password, poll new/unseen messages, save each newsletter as a Markdown source in `Inbox/`, run this ingest, and mark messages seen only after successful capture. See `references/newsletter-imap-ingestion.md`; use `scripts/newsletter_imap_capture.py` as the reusable capture-script template.
  ...
  - **Substantive newsletter issue routing:** when a daily newsletter issue contains multiple short news items, market tables, policy blurbs, and lifestyle/business snippets, archive it as one newsletter source but triage item-by-item. Create atomic pages only for durable policy/finance/social topics, update `Economic Indicators` for market/rate/oil/jobs/inflation/sentiment signals, update the newsletter-network/source-routing page when the issue teaches future routing, and leave ads/sports/trivia/routine recs source-only unless they connect to an existing durable note. Route pages to their semantic MOC (for example US policy/household-finance items can belong in `06 US Politics & Society MOC`, not just Business/Finance). For Brew Markets or other investing newsletters, apply the public-company watchlist rule: read `System/Stock Watchlist.md`, never edit it, but list clear investable ticker theses that are not already present under `Watchlist candidates (review — not added)`. For plain-body flagship Morning Brew issues, one issue can legitimately create a small cluster of durable pages (AI governance/privacy, public-company valuation/competition updates, travel/institutional operations, media/culture, commodity/market signals) while updating `Morning Brew Newsletter Network`, `Economic Indicators`, and existing company/market notes; keep ads, polls, games, trivia, recipes/recommendations, entertainment blurbs, referral blocks, and short PSA items source-only unless they connect to a durable existing page. Morning Brew Sunday specials or themed issues can also be substantive even when they lack market tables: route durable public-lands/infrastructure/tourism/culture material to semantic notes and MOCs (for example national parks capacity, domestic tourism infrastructure, or dark-sky recreation) while leaving sponsor/quiz/mailbag/referral blocks source-only; see `references/morning-brew-public-lands-routing.md`. For long-form Substack/essay newsletters (including Vietnamese sources such as Đọc Chậm), archive the issue as one source but distill the main essay into 2-4 durable concept/entity pages, route by semantic domain/region rather than a generic newsletter hub, and treat short reading-list blurbs as updates or source-only unless they establish a reusable concept; see `references/substack-essay-newsletter-routing.md`. See `references/newsletter-substantive-issue-routing.md` and `references/morning-brew-flagship-newsletter-routing.md`. When a normal writable Inbox markdown capture is moved to its normalized `Sources/` archive and then patched, re-read/check frontmatter before finalizing to catch malformed YAML from patch mistakes. **CRLF delimiter pitfall:** email/newsletter captures may use CRLF line endings; after any frontmatter patch, verify the opening and closing delimiters are exactly `---` (not `----`). If a targeted replacement leaves an extra hyphen, repair the archive/source delimiter before moving on.
  - **Scrambled-body newsletter captures with online issue links:** if the email body says the provider scrambled the email and only includes a "read it online" URL, do not treat it as low-content/admin. Preserve the original email body in the source archive, fetch/extract the online issue, then apply the substantive newsletter routing above. For Morning Brew / Tech Brew / Brew Markets pages, extracting the `<main>` HTML text after removing script/style/nav/footer boilerplate is usually enough; keep market tables and article sections, skip ads/referral boilerplate, and archive one normalized source with `online_url:` plus `## Pages Updated`. Pitfall: some plain-text links are placeholder/short paths such as `/issues/tk` that can return an HTTP-200 generic "We couldn't find that page" shell (for example title-only `Tech Brew`) rather than a hard 404, even though the issue title is present in the email. Treat generic/too-short page text as extraction failure; derive a slug from the email H1/title (lowercase, remove emoji/punctuation, possessive apostrophe-s -> `s`, spaces -> hyphens) and retry the likely canonical host, e.g. `https://www.techbrew.com/issues/<slug>` or `https://www.brewmarkets.com/issues/<slug>`, before downgrading to metadata-only. Preserve the failed raw URL in the archive body and store the successful URL as `online_url:`/`canonical_url:`. In headless cron, fetch HTML to a temporary file first (for example `curl -L -o /tmp/issue.html ...`) and parse the saved file locally; avoid `curl ... | python` / pipe-to-interpreter patterns because the approval guard treats them as downloaded-code execution and the cron job cannot approve them. See `references/newsletter-scrambled-online-extraction.md`; for the July 10 2026 Tech Brew/Brew Markets routing examples (life-ops AI prompt -> Personal/AI MOCs; Circle/OCC stablecoin banking -> stablecoin + Economic Indicators pages), see `references/newsletter-routing-session-2026-07-10.md`. For the July 11 2026 Morning Brew flagship routing example (Netflix live-channel strategy -> existing Netflix page; Meta DSA addictive-design findings -> new platform-regulation concept; one-line housing act -> existing housing-affordability page; market table -> Economic Indicators), see `references/newsletter-routing-session-2026-07-11.md`.
