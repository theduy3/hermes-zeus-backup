...
### Extension: daily investment watchlist company updates
When the user wants weekly coverage of stock-watchlist companies inside the daily investment briefing, load `stock-watchlist-vault` as well as `today`. Use the `Watchlist:` section of `/vault/System/Stock Watchlist.md` only, split tickers into weekday buckets with `index % 7` using America/Vancouver local weekday, and insert/replace exactly one `## Watchlist Company Updates (Last 30 Days)` section after Stock Watchlist and before Morning Brew in `/vault/Daily/<date>-investment.md`. Do not invent facts when web research fails; include a clear unavailable note.
