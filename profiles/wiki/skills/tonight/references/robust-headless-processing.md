...
- In scheduled cron runs, prefer explicit `read_file` / `write_file` / `terminal` steps over `execute_code` orchestration for vault mutations. If an atomic replace is needed, a simple shell `mv -f /vault/Daily/.tmp /vault/Daily/YYYY-MM-DD-tonight.md` is the low-friction path; avoid inline `python -c` replacements when the same operation can be expressed as a normal shell command. Always read the final digest back before reporting success.
