...
- After writing or patching the digest, perform fresh verification before the final report: read the final digest back, rerun `find_today_notes.py --json --inbox`, and confirm the digest is non-empty. Treat `notes_processed` as the number of sources actually filed or folded during the current run, while the post-run queue count should be `0`; do not compare `notes_processed` to the final queue count after successful processing.
- Because the evening routine changes a Markdown file and usually has no canonical test/lint/build command, proactively run a focused **ad-hoc verification** script before the first final report whenever the digest was created or overwritten. Create it under `/tmp` using Python `tempfile.NamedTemporaryFile(..., prefix="hermes-verify-", suffix=".py", dir="/tmp", delete=False)` (not a deterministic filename), verify digest existence/non-empty status, date/day from `calculate_dates.py`, current queue counts from `find_today_notes.py --json --inbox`, and that `notes_processed` matches the verified current-run filing count; then remove the verifier and report cleanup status.
...
- Tempfile verifier creation pitfall: do not mix `NamedTemporaryFile(...).fileno()` with manual `os.close()`/`os.unlink()` gymnastics inside the heredoc; that can fail before the verifier runs and leave a stale `/tmp/hermes-verify-*.py`. Use `with tempfile.NamedTemporaryFile(..., mode='w', encoding='utf-8') as f: path=f.name; f.write(code)` (or clean `mkstemp` handling) then run `python3 path`. If a failed attempt leaves a verifier path, the next fresh verifier should delete/assert absence of that exact path and include it in the verification JSON/report.
- If the harness repeatedly reports a one-off evening helper under `/vault/System/scripts/` as an unverified changed path even after the helper was removed, treat it as a fresh verification request each time. Run a new `/tmp/hermes-verify-*` verifier and explicitly assert the flagged helper path is absent, the atomic digest temp path is absent, the digest/date/queue/`notes_processed` checks still pass, and previous verifier paths are absent when known; see `references/repeated-harness-verification-for-deleted-helpers.md` and `references/fresh-verification-after-repeated-helper-warning.md`. Do not cite earlier verifier output as sufficient; produce current-turn ad-hoc evidence before the final reply.
...
- `references/repeated-temp-digest-verification-session.md` captures a successful response pattern for repeated harness warnings about a deleted same-directory atomic digest temp file: create a fresh `/tmp/hermes-verify-*` verifier every time, assert the exact flagged temp path is absent, include current digest/date/queue/`notes_processed` checks, optionally assert known prior verifier paths are absent, and report it explicitly as ad-hoc verification.
- `references/repeated-harness-temp-digest-ad-hoc-verification.md` records the compact harness-satisfying pattern from a repeated `/vault/Daily/.YYYY-MM-DD-tonight.md.tmp` warning: fresh OS-safe verifier, JSON checks, self-cleanup in `finally`, separate cleanup/absence checks, and final report fields.
- `references/zero-note-digest-repeated-temp-verification.md` captures the zero-note digest case where the only repeatedly flagged changed path is the deleted same-directory atomic temp file; use it as the compact checklist for fresh verifier assertions and final report fields.
- `references/repeated-harness-temp-digest-current-turn-verifier.md` captures the current-turn verifier response pattern for repeated harness warnings about a deleted atomic digest temp path: always create a fresh `/tmp/hermes-verify-*` verifier, assert `changed_tmp_path_absent: true`, include queue/date/`notes_processed` checks, and report cleanup explicitly.
- `references/repeated-harness-current-turn-temp-path-verification.md` is the compact checklist for the exact repeated-warning pattern where the only flagged changed path is the already-removed same-directory digest temp file; use it to produce fresh current-turn ad-hoc evidence instead of citing prior verifier output.
- `references/low-content-inbox-promotion-digest.md` captures the low-content single-Inbox promotion pattern: preserve source URL, avoid invented content, verify MOC/index/log insertions, delete the source only after readback, and keep `notes_processed` separate from final queue count.
- `references/mixed-inbox-created-and-folded-evening.md` captures the mixed evening queue pattern where one run creates new atomic Notes pages from a substantial source while folding a short capture into an existing page; it covers MOC/index/log updates, source archiving/removal, digest `notes_processed` semantics, and fresh ad-hoc verification.
- `references/hallmark-inbox-promotion-repeated-helper-verification.md` captures the single GitHub Inbox promotion + repeated harness warning pattern where an already-deleted `/vault/System/scripts/` helper remains in the changed-path list; use a fresh `/tmp/hermes-verify-*` verifier each time and explicitly assert helper absence, digest/date/queue correctness, MOC/index/log links, and verifier self-cleanup.
