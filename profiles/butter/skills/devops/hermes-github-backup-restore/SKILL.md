...
| Push fails: "could not read Username" | `GITHUB_TOKEN` missing from `~/.hermes/.env` entirely, or remote URL doesn't embed it | Add `GITHUB_TOKEN=<token>` to `.env`; verify remote: `git remote get-url origin` should show `https://<TOKEN>@github.com/...` |
