...
## Common failure mode: Token validation with Python urllib

Python's `urllib.request` can return misleading HTTP 403 with error code `1010` for valid Discord bot tokens. This is a false negative — the token may still work fine. Always validate with `curl` instead:

```bash
curl -s -H "Authorization: Bot <TOKEN>" "https://discord.com/api/v10/users/@me"
```

A valid response shows JSON with `username`, `id`, and `bot: true`.

Do NOT trust Python urllib responses for Discord token validation — use curl. See `references/discord-token-validation-curl.md` for reproduction and root cause.

## Multi-profile Discord setup

Each profile gets its own Discord bot token and gateway:

```bash
# Set per-profile env vars
# ~/.hermes/profiles/<name>/.env:
DISCORD_BOT_TOKEN=<profile-specific-token>
DISCORD_ALLOWED_USERS=<user-id>

# Install profile-scoped gateway
hermes -p <name> gateway install --force
hermes -p <name> gateway start
```

Profiles can mix platforms — some on Telegram, some on Discord, some on both — by setting only the tokens each profile needs in its `.env`.

## Token validation (BEFORE starting gateway)

Always validate Discord tokens with `curl` — Python's `urllib` can return false HTTP 403/1010:
...
- No response / timeout → network issue

Then confirm Privileged Gateway Intents are enabled in Developer Portal before starting the gateway.
